...
from camp.utils.ops import (
    generate_cluster_preference_matrix,
    generate_pi_split_preference_matrix,
...
def generate_PVRP_data(
    dataset_size, graph_size, num_agents=3, preference_type="random"
):
    loc = np.random.uniform(0, 1, size=(dataset_size, graph_size + 1, 2))
    depot = loc[:, -1]
    cust = loc[:, :-1]
    ...
    # vehicle feature
    speed = np.random.uniform(1, 1, size=(dataset_size, num_agents))
    cap = np.random.randint(40, 41, size=(dataset_size, num_agents))

    # vehicle preference matrix
    if preference_type == "random":
        preference = generate_random_preference_matrix(depot, cust, num_agents)
    elif preference_type == "pi":
        preference = generate_pi_split_preference_matrix(depot, cust, num_agents)
    elif preference_type == "cluster":
        preference = generate_cluster_preference_matrix(depot, cust, num_agents)
    else:
        raise ValueError(f"Preference type {preference_type} not implemented")
