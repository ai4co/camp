...
from camp.models.agent_handlers import RandomAgentHandler
...
class ParallelRandomInsertionHeuristic(Heuristic):
    ...
    def get_action(self, td):
        ...
        actions = self.agent_handler(actions, current_loc_idx, td)[
            0
        ]  # handle conflicts
        return actions
    ...
    def get_action(self, td):
        if td["i"][0].item() == 0:
            ...
            for i in range(self.num_agents):
                ...
                cur_dist[torch.arange(cur_dist.shape[0]), td["current_node"][:, i]] = (
                    float(100000)
                )
                action = cur_dist.argmin(dim=-1)
                available.scatter_(
                    -1, action.unsqueeze(-1), False
                )  # update action mask
                actions.append(action)
