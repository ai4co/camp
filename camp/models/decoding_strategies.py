...
class CAMPDecodingStrategy(metaclass=abc.ABCMeta):
    name = "base"
    ...
    def __init__(
        ...
        if store_all_logp and mask_handled:
            raise ValueError(
                "store_all_logp is not supported when mask_handled is True"
            )
...
class Greedy(CAMPDecodingStrategy):
    name = "greedy"
...
class Sampling(CAMPDecodingStrategy):
    name = "sampling"
...
class Evaluate(CAMPDecodingStrategy):
    name = "evaluate"
...
class CAMP4FFSPDecoding:
    def __init__(self, stage_idx, num_ma, num_job, use_pos_token: bool = False) -> None:
    ...
    def step(
        ...
        while not mask[..., :-1].all():
            ...
            idle_machines = idle_machines[
                idle_machines != selected_stage_machine[:, None]
            ].view(*batch_size, -1)
        ...
        if len(jobs_selected) > 0:
            ...
            stage_mas_selected = torch.stack(stage_mas_selected, dim=-1).view(
                *batch_size, -1
            )
            actions_probs = torch.stack(actions_probs, dim=-1).view(*batch_size, -1)

            actions = TensorDict(
                {"jobs": jobs_selected, "mas": mas_selected},
