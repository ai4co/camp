...
from camp.envs import FFSPEnv

from .agent_handlers import get_agent_handler
from .decoder import MatNetDecoder, CAMPDecoder
from .decoding_strategies import (
    CAMP4FFSPDecoding,
    CAMPDecodingStrategy,
    parco_get_decoding_strategy,
)
from .encoder import MatNetEncoder, CAMPEncoder
...
class CAMPPolicy(nn.Module):
    """Policy for CAMP model"""
    ...
    def __init__(
        ...
        multi_objective: bool = True,  # Multi-objective optimization
        multi_objective_weights: List[float] = [1, 0],  # Multi-objective weights
    ):
        super(CAMPPolicy, self).__init__()
        ...
        # Encoder and decoder
        if encoder is None:
            log.info("Initializing default CAMPEncoder")
            self.encoder = CAMPEncoder(
        ...
        if decoder is None:
            log.info("Initializing default CAMPDecoder")
            self.decoder = CAMPDecoder(
                embed_dim=embed_dim,
                num_heads=num_heads,
    ...
    def forward(
        ...
        **decoding_kwargs,
        ...
        decode_strategy: CAMPDecodingStrategy = parco_get_decoding_strategy(
            decode_type,
        ...
        if calc_reward:
            if self.multi_objective:
                time, preference = env.get_reward(td, actions)
                reward = (
...
class CAMPMultiStagePolicy(nn.Module):
    ...
    def forward(
        ...
        **decoder_kwargs,
        ...
        while not td["done"].all():
            actions = torch.full(
                size=(bs, total_mas), fill_value=num_jobs, device=device
            )
            prob_stack = []
            action_stack = []
            for stage_idx in range(self.stage_cnt):
...
class OneStageModel(nn.Module):
    ...
    def pre_forward(self, td, env, num_starts):
        ...
        self.decoding_strat = CAMP4FFSPDecoding(
            self.stage_idx,
            num_ma=self.num_ma,
