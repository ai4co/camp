...
from camp.models.nn.transformer import Normalization, ParallelGatedMLP
...
class TransformerFFN(nn.Module):
    def __init__(
        ...
        if parallel_gated_kwargs is not None:
            ...
            ffn = FeedForward(
                embed_dim=embed_dim, feedforward_hidden=feedforward_hidden
            )

        self.ops = nn.ModuleDict(
            {
                "norm1": Normalization(
                    embed_dim=embed_dim, normalization=normalization
                ),
                "ffn": ffn,
                "norm2": Normalization(
                    embed_dim=embed_dim, normalization=normalization
...
class MatNetBlock(nn.Module):
    def __init__(
        ...
        self.mixed_score_MHA = MixedScoreMultiHeadAttention(
            embed_dim=embed_dim, head_num=head_num, ms_hidden_dim=ms_hidden_dim
        )
...
class EfficientMixedScoreMultiHeadAttention(nn.Module):
    ...
    def forward(self, x1, x2, attn_mask=None, cost_mat=None):
        ...
        h1 = self.out_proj1(apply_weights_and_combine(ms1, v2, scale=self.scale_dots))
        h2 = self.out_proj2(
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.cross_attn = EfficientMixedScoreMultiHeadAttention(
            embed_dim=embed_dim, num_heads=num_heads, ms_hidden_dim=ms_hidden_dim
        )
