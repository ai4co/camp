...
class CAMPDecoder(AttentionModelDecoder):
    def __init__(
        self,
        embed_dim: int = 128,
        num_heads: int = 8,
        ...
        **kwargs,
        ...
        if use_graph_context:
            raise ValueError("CAMP does not use graph context")

        super(CAMPDecoder, self).__init__(
            embed_dim=embed_dim,
            num_heads=num_heads,
            env_name=env_name,
...
class MatNetDecoder(CAMPDecoder):
    def __init__(
        self,
        stage_idx: int,
        stage_cnt: int,
        ...
        **kwargs,
        ...
        dynamic_embedding_kwargs.update({"embed_dim": embed_dim})
...
class PVRPDecoder(AttentionModelDecoder):
    def __init__(
        self,
        embed_dim: int = 128,
        num_heads: int = 8,
        ...
        **kwargs,
        ...
        context_embedding_kwargs["embed_dim"] = embed_dim  # replace
        if context_embedding is None:
            context_embedding = env_context_embedding("PVRP", context_embedding_kwargs)

        if dynamic_embedding is None:
            dynamic_embedding = env_dynamic_embedding("PVRP", dynamic_embedding_kwargs)
    ...
    def forward(
        ...
        # Compute logits
        # Note that in this case we should consider [B*M] as a batch dimension
        # since embeddings are different. Usually, we used a query of [B, M, H], but
        # in this case, each embedding is different for each agent, so we need to actually have
        # [B*M, 1, H] as a query (same goes for K,V,L)
        b, m, mn, hdim = glimpse_k.shape
        glimpse_q = glimpse_q.view(b * m, 1, hdim)
        glimpse_k = glimpse_k.view(b * m, mn, hdim)
        glimpse_v = glimpse_v.view(b * m, mn, hdim)
        logit_k = logit_k.view(b * m, mn, hdim)
        mask = mask.view(b * m, 1, mn)
        logits = self.pointer(glimpse_q, glimpse_k, glimpse_v, logit_k, mask)

        # For passing to the next step communication layer, reshape logits and mask to [B*S, m, N] if num_starts > 1
        if num_starts > 1:
            logits = logits.reshape(batch_size * num_starts, -1, logits.shape[-1])
            mask = mask.reshape(batch_size * num_starts, -1, mask.shape[-1])
            glimpse_q = glimpse_q.reshape(
        ...
        return logits.view(b, m, -1), mask.view(b, m, -1)
...
class PVRPHardCodeDecoder(AttentionModelDecoder):
    def __init__(
        self,
        embed_dim: int = 128,
        num_heads: int = 8,
        ...
        **kwargs,
        ...
        super(PVRPHardCodeDecoder, self).__init__(
            embed_dim=embed_dim,
            num_heads=num_heads,
