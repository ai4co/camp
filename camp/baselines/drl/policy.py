...
from camp.baselines.drl.problems.PVRP.problem_PVRP import HCVRP
...
class DRLPolicy(nn.Module):
    def __init__(
        ...
        super(DRLPolicy, self).__init__()

        self.embed_dim = embed_dim

        self.env = env
        self.problem = HCVRP(max_veh=max_veh)

        self.drl_net = AttentionModel(
            embedding_dim=embed_dim,
            hidden_dim=embed_dim,
            obj=obj,
            problem=self.problem,
            multi_objective_weights=multi_objective_time_weights,
            max_veh=max_veh,
            max_nodes=max_nodes,
        )
    ...
    def forward(
        ...
        **decoding_kwargs,
        ...
        if return_actions:
            cost, time, preference, ll, pi, veh = self.drl_net(
            ...
            cost, time, preference, ll = self.drl_net(input=inputs, return_pi=False)
            outdict = {
                "reward": -cost.squeeze(),
                "time": time,
                "preference": preference,
