...
def load_problem(name):
    ...
    problem = {"hcvrp": HCVRP}.get(name, None)
...
def load_model(path, obj, epoch=None):
    from camp.baselines.drl.nets.attention_model import AttentionModel
    from camp.baselines.drl.nets.pointer_network import PointerNetwork
...
def sample_many(inner_func, get_cost_func, input, batch_rep=1, iter_rep=1):
    ...
    for i in range(iter_rep):
        _log_p, _log_p_veh, pi, veh_list, tour_1, tour_2, tour_3, tour_4, tour_5 = (
            inner_func(
                input,
            )
        )
        ...
        cost, mask = get_cost_func(
            input, pi, veh_list, tour_1, tour_2, tour_3, tour_4, tour_5
        )
        costs.append(cost.view(batch_rep, -1).t())  # [1, (num_samples, batch_rep)]
        veh_lists.append(
            veh_list.view(batch_rep, -1, veh_list.size(-1)).transpose(0, 1)
        )  # [1, (num_samples, batch_rep, solu_len)]
        pis.append(
            pi.view(batch_rep, -1, pi.size(-1)).transpose(0, 1)
        )  # [1, (num_samples, batch_rep, solu_len)]
    ...
    minveh = veh_lists[
        torch.arange(veh_lists.size(0), out=argmincosts.new()), argmincosts
    ]
