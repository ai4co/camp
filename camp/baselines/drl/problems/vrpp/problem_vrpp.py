...
from camp.baselines.drl.utils.beam_search import beam_search

# from problems.hcvrp.state_hcvrp import StateHCVRP
from .state_PVRP import StateHCVRP
...
class HCVRP(object):
    ...
    def __init__(self, max_veh):
        self.max_veh = max_veh

    @staticmethod
    def get_costs(
        dataset, obj, pi, veh_list, tour_1, tour_2, tour_3, tour_4, tour_5
    ):  # pi is a solution sequence, [batch_size, num_veh, tour_len]
        if obj == "min-max":
            SPEED = [1, 1, 1, 1, 1]
        if obj == "min-sum":
        ...
        demand_with_depot = torch.cat(  # [batch_size, graph_size]
            (
                torch.full_like(
                    dataset["demand"][:, :1], 0
                ),  # pickup problem, set depot demand to -capacity
                dataset["demand"],
            ),
            1,
        )
        ...
        for i in range(pi.size(-1)):  # tour_len
            used_cap[
                torch.arange(batch_size), veh_list[torch.arange(batch_size), i]
            ] += d[
                :, i
            ...
            assert (
                used_cap[torch.arange(batch_size), 0]
                <= HCVRP.VEHICLE_CAPACITY[0] + 1e-5
            ).all(), "Used more than capacity 1"
            used_cap[(tour_2[:, i] == 0), 1] = 0
            assert (
                used_cap[torch.arange(batch_size), 1]
                <= HCVRP.VEHICLE_CAPACITY[1] + 1e-5
            ).all(), "Used more than capacity 2"
            used_cap[(tour_3[:, i] == 0), 2] = 0
            assert (
                used_cap[torch.arange(batch_size), 2]
                <= HCVRP.VEHICLE_CAPACITY[2] + 1e-5
            ).all(), "Used more than capacity 3"
            used_cap[(tour_4[:, i] == 0), 3] = 0
            assert (
                used_cap[torch.arange(batch_size), 3]
                <= HCVRP.VEHICLE_CAPACITY[3] + 1e-5
            ).all(), "Used more than capacity 4"
            used_cap[(tour_5[:, i] == 0), 4] = 0
            assert (
                used_cap[torch.arange(batch_size), 4]
                <= HCVRP.VEHICLE_CAPACITY[4] + 1e-5
            ).all(), "Used more than capacity 5"
        ...
        dis_1 = loc_with_depot.gather(
            1, tour_1[..., None].expand(*tour_1.size(), loc_with_depot.size(-1))
        )
        dis_2 = loc_with_depot.gather(
            1, tour_2[..., None].expand(*tour_2.size(), loc_with_depot.size(-1))
        )
        dis_3 = loc_with_depot.gather(
            1, tour_3[..., None].expand(*tour_3.size(), loc_with_depot.size(-1))
        )
        dis_4 = loc_with_depot.gather(
            1, tour_4[..., None].expand(*tour_4.size(), loc_with_depot.size(-1))
        )
        dis_5 = loc_with_depot.gather(
            1, tour_5[..., None].expand(*tour_5.size(), loc_with_depot.size(-1))
        )

        total_dis_1 = (
            (
                (dis_1[:, 1:] - dis_1[:, :-1]).norm(p=2, dim=2).sum(1)
                + (dis_1[:, 0] - dataset["depot"]).norm(p=2, dim=1)  # Depot to first
                + (dis_1[:, -1] - dataset["depot"]).norm(p=2, dim=1)
            )
            / SPEED[0]
        ).unsqueeze(-1)  # [batch_size]
        total_dis_2 = (
            (
                (dis_2[:, 1:] - dis_2[:, :-1]).norm(p=2, dim=2).sum(1)
                + (dis_2[:, 0] - dataset["depot"]).norm(p=2, dim=1)  # Depot to first
                + (dis_2[:, -1] - dataset["depot"]).norm(p=2, dim=1)
            )
            / SPEED[1]
        ).unsqueeze(-1)  # [batch_size]
        total_dis_3 = (
            (
                (dis_3[:, 1:] - dis_3[:, :-1]).norm(p=2, dim=2).sum(1)
                + (dis_3[:, 0] - dataset["depot"]).norm(p=2, dim=1)  # Depot to first
                + (dis_3[:, -1] - dataset["depot"]).norm(p=2, dim=1)
            )
            / SPEED[2]
        ).unsqueeze(-1)  # [batch_size]
        total_dis_4 = (
            (
                (dis_4[:, 1:] - dis_4[:, :-1]).norm(p=2, dim=2).sum(1)
                + (dis_4[:, 0] - dataset["depot"]).norm(p=2, dim=1)  # Depot to first
                + (dis_4[:, -1] - dataset["depot"]).norm(p=2, dim=1)
            )
            / SPEED[3]
        ).unsqueeze(-1)  # [batch_size]
        total_dis_5 = (
            (
                (dis_5[:, 1:] - dis_5[:, :-1]).norm(p=2, dim=2).sum(1)
                + (dis_5[:, 0] - dataset["depot"]).norm(p=2, dim=1)  # Depot to first
                + (dis_5[:, -1] - dataset["depot"]).norm(p=2, dim=1)
            )
            / SPEED[4]
        ).unsqueeze(-1)  # [batch_size]

        total_dis = torch.cat(
            (total_dis_1, total_dis_2, total_dis_3, total_dis_4, total_dis_5), -1
        )
    ...
    @staticmethod
    def make_state(num_agents, inputs):
        return StateHCVRP.initialize(num_agents, inputs)
    ...
    def __init__(
        ...
        if filename is not None:
            ...
            CAPACITIES = {
                ...
                80: [20.0, 25.0, 30.0, 35.0, 40.0],
                100: [20.0, 25.0, 30.0, 35.0, 40.0],
                120: [20.0, 25.0, 30.0, 35.0, 40.0],
                140: [20.0, 25.0, 30.0, 35.0, 40.0],
                160: [20.0, 25.0, 30.0, 35.0, 40.0],
            }
