...
class DecodingStrategy(metaclass=abc.ABCMeta):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.actions = []
        self.logprobs = []
        self.dist_alpha = nn.Parameter(torch.Tensor([1.0]), requires_grad=True)
    ...
    def pre_decoder_hook(
        ...
        if self.num_starts >= 1:
            if self.multistart:
                if (
                    action is None
                ):  # if action is provided, we use it as the first action
    ...
    def step(
        ...
        **kwargs,
        ...
        curr_duration = td["current_length"] / td["agents_speed"]
        curr_duration = gather_by_index(curr_duration, td["current_agent_idx"], dim=1)
        logits_bias = self.dist_alpha * curr_duration
