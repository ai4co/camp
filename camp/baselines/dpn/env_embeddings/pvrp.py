...
from camp.baselines.dpn.nn.positional_encoder import RotatePostionalEncoding
...
class PVRPSeqInitEmbedding(nn.Module):
    def __init__(
        ...
        """
        Profiled Vehicle Routing Problem sequential version (PVRPSeq) embedding.
        """
        super(PVRPSeqInitEmbedding, self).__init__()
        self.embed_dim = embed_dim
        self.init_embed_depot = nn.Linear(2, embed_dim, linear_bias)
        self.init_embed_agent = nn.Linear(2, embed_dim, linear_bias)
        self.pos_encoder = RotatePostionalEncoding(embed_dim, 10000)
    ...
    def forward(self, td):
        ...
        place_embedding = self.init_embed_agent(depot_locs)

        pos_embedding = self.pos_encoder(depots_embedding)
