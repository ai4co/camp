...
from .generator import PVRPZCGenerator
from .render import render

log = get_pylogger(__name__)


class PVRPZCSeqEnv(RL4COEnvBase):
    """Profiled Vehicle Routing Problem with Preferences (PVRP-Preference) Environment and Zone constraint (Sequential Version)."""

    name = "PVRP_zone"

    def __init__(
        self,
        generator: PVRPZCGenerator = None,
        generator_params: dict = {},
        **kwargs,
    ):
        super().__init__(**kwargs)
        if generator is None:
            generator = PVRPZCGenerator(**generator_params)
    ...
    def _reset(
        ...
        # )
        # Depot preference is an eye matrix
        preference_depot = (
            torch.eye(num_agents, device=device).unsqueeze(0).repeat(*batch_size, 1, 1)
        )
    ...
    def get_action_mask(td: TensorDict) -> torch.Tensor:
        ...
        # Action mask will be matched with the prefernece matrix
        action_mask &= td["agents_preference"]
    ...
    def _make_spec(self, generator: PVRPZCGenerator):
